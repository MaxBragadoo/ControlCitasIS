...
public class FrmPermisos extends javax.swing.JFrame {
    ...
    private String nombreFormulario = "Permisos";
    Permiso objeto;
    ...
    private void inicializarForm(Permiso o) {
        ...
        txtCodigo.setText(o.getCodigoPermiso());
    ...
    private Permiso createObject() {
        this.objeto.setCodigoPermiso(txtCodigo.getText().trim());
    ...
    private void editar() {
        int id = getId();
        if (id > 0) {
            inicializarForm(PermisoBLL.find(id));
        }
    }
    ...
    private void eliminar() {
        int id = getId();
        if (id > 0) {
            if (PermisoBLL.delete(id)) {
                Utils.showInformation("Permiso eliminado");
                listar();
            } else {
                Utils.showExcepcion();
            }
    ...
    private void baja() {
        int id = getId();
        if (id > 0) {
            Permiso o = PermisoBLL.find(id);
            o.setEstatus("Baja");
            if (PermisoBLL.edit(o)) {
    ...
    private void guardar() {
        if (validateForm()) {
            if (registrar) {
                if (PermisoBLL.add(createObject())) {
                    Utils.showInformation("Permiso registrado");
                    nuevo();
                    listar();
                } else {
                    Utils.showExcepcion();
                ...
                if (PermisoBLL.edit(createObject())) {
                    Utils.showInformation("Permiso actualizado");
                    listar();
                } else {
                    Utils.showExcepcion();
                }
    ...
    private void listar() {
        if (_btnBuscar) {
            ...
            tabla.setModel(PermisoBLL.createTable((DefaultTableModel) tabla.getModel(), PermisoBLL.list(filter)));
        }
    }
    ...
    private void initComponents()
    {
        ...
        jLabel7.setText("Buscar por CODIGO PERMISO o NOMBRE");
        ...
        tabla.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String []
            {
                "ID", "COD. PERMISO", "NOMBRE", "DESCRIPCION", "ESTATUS", "FECHA ALTA"
            }
        )
        {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FrmPermisos().setVisible(true);
            }
        });
    }
